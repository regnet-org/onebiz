...
class WebsiteController extends Controller
{
    ...
    {
        if(0 == 1 && ($_SERVER['REMOTE_ADDR']=="82.78.230.101" || $_SERVER['REMOTE_ADDR']=="192.168.1.3")) {
            Auth::loginUsingId(559);
    ...
    public function send_message(Request $request)
    {
     ...
     public function send_regcomert_message(Request $request)
    {
       @ini_set('max_execution_time', 0);
       @set_time_limit(0);
       Overrider::load("Settings");
       ...
       $this->validate($request, [
        ...
        if(get_option('contact_email') != ''){
            try{
                Mail::to(get_option('contact_email'))->send(new RegComertMail($mail));      
                echo json_encode(array('result'=>true,'message'=>_lang('Your Message send sucessfully.')));
            }catch (\Exception $e) {
                echo json_encode(array('result'=>false,'message'=>_lang('Error Occured, Please try again !')));
            }        
