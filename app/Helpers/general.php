...
if ( ! function_exists('has_permission')){
	function has_permission($name, $user_id) 
	{
		$permission = DB::table('permissions')
					    ...
					    ->where('staff_company_id', session('staff_company_id'))
...
if ( ! function_exists('permission_list')){
	...
	{	  
		if(!session('staff_company_id'))
			session(['staff_company_id' => Auth::user()->company_id]);
		elseif(request()->staff_company_id)
			session(['staff_company_id' => request()->staff_company_id]);
...
if ( ! function_exists('company_id')){
	...
	{
		if(Auth::check()){
			if( Auth::user()->company_id != '' && Auth::user()->user_type == 'staff' && session('staff_company_id')){	 
				return session('staff_company_id');
...
if ( ! function_exists('invoice_status')){
	function invoice_status($status)
	{
		if($status == 'Unpaid'){
			...
			return "<span class='badge badge-dark'>"._lang($status)."</span>"; 
		}
...
if ( ! function_exists('project_status')){
	function project_status($status)
	{
		...
		if($status == 'not_started'){
			return "<span class='badge badge-info'>"._lang($uc_status)."</span>"; 
		}else if($status == 'in_progress'){
			return "<span class='badge badge-primary'>"._lang($uc_status)."</span>"; 
		}else if($status == 'on_hold'){
			return "<span class='badge badge-warning'>"._lang($uc_status)."</span>"; 
		}else if($status == 'cancelled'){
			return "<span class='badge badge-danger'>"._lang($uc_status)."</span>"; 
		}else if($status == 'completed'){
			return "<span class='badge badge-success'>"._lang($uc_status)."</span>"; 
		}
	}
}
...
if ( ! function_exists('task_priority')){
	...
	{
		if($priority == 'low'){
			return "<span class='badge badge-primary'>".ucwords(_lang($priority))."</span>"; 
		}else if($priority == 'medium'){
			return "<span class='badge badge-warning'>".ucwords(_lang($priority))."</span>"; 
		}else if($priority == 'high'){
			return "<span class='badge badge-danger'>".ucwords(_lang($priority))."</span>"; 
		}
	}
}
