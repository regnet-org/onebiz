			    ...
			    <tr id="row_{{ $user->id }}">
					...
					<td class="text-center">
					  <form action="{{action('StaffController@destroy', $user['id'])}}" method="post">
						@if($user->company_id==company_id())<a href="{{action('StaffController@edit', $user['id'])}}" data-title="{{ _lang('Update Staf') }}" class="btn btn-warning btn-xs ajax-modal"><i class="ti-pencil"></i></a>@endif
