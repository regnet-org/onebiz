		...
		<div class="card clearfix">
			...
			<div class="card-body">
				<div id="invoice-view">
					...
					@php $base_currency = base_currency(); 
					@endphp
							 ...
							 <tbody id="invoice">
								 @foreach($invoice->invoice_items as $item)
									<tr id="product-{{ $item->item_id }}">
										...
										<td class="text-right">{{ $client_currency != $base_currency ? decimalPlace(convert_currency($base_currency, $client_currency, $item->unit_cost), currency($client_currency)) : decimalPlace($item->unit_cost, $currency) }}</td>
										<td class="text-right">{{ $client_currency != $base_currency ? decimalPlace(convert_currency($base_currency, $client_currency, $item->discount), currency($client_currency)) : decimalPlace($item->discount, $currency) }}</TD>
										<td class="no-print">{{ strtoupper(__('general.'.$item->tax_method)) }}</td>
										<td class="text-right">{{ $client_currency != $base_currency ? decimalPlace(convert_currency($base_currency, $client_currency, $item->tax_amount), currency($client_currency)) : decimalPlace($item->tax_amount, $currency) }}</TD>
										<td class="text-right">{{ $client_currency != $base_currency ? decimalPlace(convert_currency($base_currency, $client_currency, $item->sub_total), currency($client_currency)) : decimalPlace($item->sub_total, $currency) }}</TD>
									</tr>
								 @endforeach
							 </tbody>
						</table>
...
@php 
$invoice_details_storno = invoice_details($invoice->storno_invoice_id); 
@endphp
...
@endif
						...
						<table class="table table-bordered" id="invoice-summary-table">
							 <tbody>
										 ...
										 <td>{{ _lang('Tax') }}</td>
										  <td class="text-right">
											@if($client_currency != $base_currency)
												<span>{{ decimalPlace(convert_currency($base_currency, $client_currency, $invoice->tax_total), currency($client_currency)) }}</span><br>	
											@endif
									...
									<tr>
										 ...
										 <td class="text-right">
											 @if($client_currency != $base_currency)
												<b>{{ decimalPlace($invoice->converted_total, currency($client_currency)) }}</b><br>
											 @endif
									...
									<tr>
										 ...
										 <td class="text-right">
											@if($client_currency != $base_currency)
												<span>{{ decimalPlace(convert_currency($base_currency, $client_currency, $invoice->paid), currency($client_currency)) }}</span>	<br>
											@endif
					...
					@if(get_company_option('invoice_footer')  != '')
						<div class="invoice-note">{!! clean(get_company_option('invoice_footer')) !!}</div>
					@endif
					<!--End Invoice Note-->
				</div>
			</div>
