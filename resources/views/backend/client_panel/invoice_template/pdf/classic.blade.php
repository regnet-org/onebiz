			 ...
			 <tbody id="invoice">
				 @foreach($invoice->invoice_items as $item)
					 <tr id="product-{{ $item->item_id }}">
						...
						<td class="text-right">{{ $client_currency != $base_currency ? decimalPlace(convert_currency($base_currency, $client_currency, $item->unit_cost),$client_currency) : decimalPlace($item->unit_cost, $currency) }}</td>
						<td class="text-right">{{ $client_currency != $base_currency ? decimalPlace(convert_currency($base_currency, $client_currency, $item->discount), $client_currency) : decimalPlace($item->discount, $currency) }}</TD>
						<td class="no-print">{{ strtoupper(__('general.'.$item->tax_method)) }}</td>
						<td class="text-right">{{ $client_currency != $base_currency ? decimalPlace(convert_currency($base_currency, $client_currency, $item->tax_amount), $client_currency) : decimalPlace($item->tax_amount, $currency) }}</TD>
						<td class="text-right">{{ $client_currency != $base_currency ? decimalPlace(convert_currency($base_currency, $client_currency, $item->sub_total), $client_currency) : decimalPlace($item->sub_total, $currency) }}</TD>
					 </tr>
				 @endforeach
			 </tbody>
		</table>
