<form method="post" class="ajax-submit" autocomplete="off" action="{{route('users.store')}}" enctype="multipart/form-data">
	...
	<div class="row p-2">
		...
		<div class="col-md-6">
		  <div class="form-group">
			<label class="control-label">{{ _lang('Allow uploading after days 10 of current month') }}</label>						
			<select class="form-control" id="allow_upload_after_10" name="allow_upload_after_10" required>
		...
		<div class="col-md-6">
		  <div class="form-group">
			<label class="control-label">{{ _lang('Allow to delete files in FileManager') }}</label>						
			<select class="form-control" id="allow_file_manager_delete" name="allow_file_manager_delete" required>
				<option value="0">{{ _lang('No') }}</option>
				<option value="1">{{ _lang('Yes') }}</option>
			</select>
		  </div>
		...
		<div class="col-md-12">
		 <div class="form-group">
			...
			<input type="file" class="dropify" name="profile_picture" data-allowed-file-extensions="png jpg jpeg PNG JPG JPEG" data-default-file="" data-max-file-size="1M"  data-max-width="300">
		 </div>
		</div>
