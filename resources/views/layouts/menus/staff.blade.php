...
@if(count(explode("|", trim(Auth::user()->company_ids, "|")))>1)
<li>
	<div class="text-warning text-center mb-2">{{_lang('Select the Company')}}</div>
	<div class="float-right" title="{{_lang('Remove me from this company')}}"><a href="{{route('staffLeaveCompany')}}" onClick="return confirm('{{_lang('Do you want to stop collaboration with this company')}}')"><i class="fa fa-trash"></i></a></div>
	<form action="{{ action('DashboardController@index') }}" class="text-center" method="get">
	<select name="staff_company_id" id="staff_company_id" onchange="this.form.submit()" class="form-control w-75 mx-auto">
	    @foreach(\App\Company::select('id', 'business_name')->whereIn('id', explode("|", trim(Auth::user()->company_ids, "|")))->orderBy('business_name')->get() as $company)
	    	<option value="{{$company->id}}" @if($company->id==session('staff_company_id'))selected="selected"@endif>{{$company->business_name}}</option>
	    @endforeach
	</select>
	</form>
</li>
